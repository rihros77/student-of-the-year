...
import {
  Clock, TrendingUp, TrendingDown, BookOpen, Dumbbell, Music,
  Code, Users, Loader2, AlertTriangle
} from 'lucide-react';
...
const TransactionItem = ({ transaction, isLast }) => {
  const { points, category, reason, created_at, event } = transaction; // ✅ FIXED
  const categoryInfo = CATEGORY_ICONS[category?.toLowerCase()] || {
  ...
  const fetchTimelineData = async () => {
    ...
    try {
      const data = await apiFetch(
        `${API_BASE_URL}/students/${PLACEHOLDER_STUDENT_ID}/timeline`
      );
    ...
    return (
      <div className="p-8 text-center bg-red-50 border border-red-200 rounded-xl shadow-lg m-4">
        ...
        <p className="text-red-600 mt-2">
          Could not fetch the student's point history. ({error})
        </p>
        <button
