...
export default function PointBreakdown() {
  ...
  const allCategoryPoints = totals ? CATEGORY_MAP.map((c) => totals[c.key] || 0) : [];
  const maxCategoryPoints = Math.max(...allCategoryPoints, 1);

  // Loading / Error / No Data states
  if (loading) return <Loading />;
  if (error) return <Error message={error} />;
  if (!student || !totals) return <NoData />;
        ...
        <div className="bg-white p-6 rounded-xl shadow-xl">
          ...
          <div className="space-y-4">
            {CATEGORY_MAP.map((category) => {
              ...
              return (
                <div key={category.key} className="p-4 border border-gray-100 rounded-lg transition duration-300 hover:shadow-md">
